...
Wenn ein Lauf in einer Sandbox, einem CI-Job, Editor-Agenten oder ähnlichen Harness mit einer Meldung wie `command failed because it timed out` abbricht, ist das kein Pytest-Fehler und kein Timeout aus `scripts/run_tests.sh`. Der Test-Runner setzt bewusst keinen äußeren Shell-Timeout mehr. Diese Meldung kommt von der äußeren Ausführungsumgebung, die den gesamten Prozess beendet hat. In diesem Fall nicht versuchen, im Projekt einen Workaround für diese genaue Meldung zu bauen; stattdessen den betroffenen Lauf kleiner machen, lokal mit `./scripts/run_tests.sh debug ...` eingrenzen oder das Timeout/Limit der Sandbox beziehungsweise CI-Umgebung anpassen.
