...
```bash
nexusctl work plan <request_id> --repo <repo_id> --branch <branch> --assign <agent_id> --sanitized-summary "..." --output json
...
nexusctl work assign <request_id> --agent sw-builder --output json
```
...
Reviewers use `nexusctl reviews submit`. Techlead or Nexus advances lifecycle only when gates are satisfied.
