...
def test_session_store_writes_to_selected_agent_dir(tmp_path):
    env = {
...
def test_session_store_prefers_selected_agent_dir_when_multiple_dirs_exist(tmp_path):
    base = tmp_path / "agents"
    ...
    payload_secondary = dict(payload_main)
    payload_secondary["session_id"] = "S-2026-secondary"
    ...
    (d2 / "current.json").write_text(json.dumps(payload_secondary), encoding="utf-8")
