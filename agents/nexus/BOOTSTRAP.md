...
2. Confirm runtime workspace path: prefer `/workspace` in container runtime; otherwise use current runtime workspace and log it.
3. Load current GitHub lifecycle context and open handoff status queue for this role.
4. Run preflight checks for schema completeness, duplicate identity, SLA urgency, and ownership clarity.
5. Pick exactly one highest-impact lifecycle action in scope.
